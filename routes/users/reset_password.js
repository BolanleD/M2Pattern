...
router.post('/login/forgot', (req, res, next) => {
  async.waterfall([
    ...
    (token, user, done) => {
      ...
      let mailOptions = {
        to: user.email,
        from: 'm2pattern@yahoo.com',
        subject: 'Password Reset',
        html: 'You are receiving this because you (or someone else) have requested the reset of the password for your account.\n\n' +
            'Please click on the following link, or paste this into your browser to complete the process:\n\n' +
            '<a href="http://' + req.headers.host + '/users/reset/' + token +'">Reset Your Pasword</a>\n\n' +
            'If you did not request this, please ignore this email and your password will not be changed.\n',
          };
      smtpTransport.sendMail(mailOptions, err => {
        console.log(mailOptions)
        req.flash('success', `We sent a password reset email to ${user.email} with further instructions.`);
